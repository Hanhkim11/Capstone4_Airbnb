...
export default function UserList() {
  ...
  const handleAddAdmin = async (e: React.FormEvent) => {
    ...
    try {
      if (editingUser) {
        ...
        await apiAdmin.updateUserService(editingUser.id, userPayload);
        toast.success("Cập nhật người dùng thành công!");
      } else {
        // Create new admin
        const addRes = await apiAdmin.addUserService(userPayload);
        let newUser = addRes?.content || addRes;
      ...
      toast.error(error.response?.data?.content || (editingUser ? "Cập nhật người dùng thất bại!" : "Thêm admin thất bại!"));
  ...
  const handleDelete = async (id: number) => {
    if (window.confirm("Bạn có chắc muốn xóa người dùng này?")) {
      try {
        await apiAdmin.deleteUserService(id);
